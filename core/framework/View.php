...
class View {
	...
	public function shine(){
		...
		$this->dwoo->setCompileDir('core/views/template_compile');
	...
	public function render($header_footer, $body){
		...
		$core_modulo = $this->universe->get_core_module();
		$this->assign('core_modulo', $core_modulo);
		$this->set_todo();

		$prefix = 'views/';

		if($core_modulo == 'core/modulos'){
			$prefix = 'core/views/';
		}
		...
		if(!empty($header_footer)){
			$header = new \Dwoo\Template\File($prefix . rtrim(strtolower($header_footer), '/') . '/header.html');
		}

		$body = new \Dwoo\Template\File($this->definir_arquivo_body($body));

		if(!empty($header_footer)){
			$footer = new \Dwoo\Template\File($prefix . rtrim(strtolower($header_footer), '/') . '/footer.html');
		}
		...
		if(isset($this->lazy_view) && !empty($this->lazy_view)){
			$lazy_view = new \Dwoo\Template\File($prefix . 'back/form_padrao/lazy_view.html');
	...
	private function definir_arquivo_body($body){
		...
		if(file_exists($core_modulo . '/' . strtolower($body) . '.html')){
			return $core_modulo . '/' . strtolower($body) . '.html';
		}

		$caminho_cruzado = [
			'framework/modulos' => 'modulos',
			'modulos'           => 'core/framework/modulos'
		];

		if(file_exists($caminho_cruzado[$core_modulo] . '/' . strtolower($body) . '.html')){
			$this->assign('core_modulo', $caminho_cruzado[$core_modulo]);
			return $caminho_cruzado[$core_modulo] . '/' . strtolower($body) . '.html';
		}
	...
	private function clear_template_compile_folder(){
		$folder = 'core/views/template_compile';
		$files = glob($folder . '/*');
	...
	public function render_plataforma_arquivo($identificador){
		...
		if(isset($this->lazy_view) && !empty($this->lazy_view)){
			$lazy_view = new \Dwoo\Template\File('core/views/back/form_padrao/lazy_view.html');
			echo $this->dwoo->get($lazy_view, $this->assign);
		}
